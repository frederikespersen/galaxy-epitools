...
def calc_tap(schro_prot_desc: pd.DataFrame) -> pd.DataFrame:
    """
...
def calc_tada(schro_prot_desc: pd.DataFrame) -> pd.DataFrame:
    """
    Takes a Protein Descriptors dataframe,
    ...
    return pd.DataFrame({'TA-DA': tada}, index=schro_prot_desc.index)
...
def developability_measures(schroprotdesc_csv: str,
    ...
    schro_prot_desc.index = schro_prot_desc.index.map(lambda s: s.split(' ')[0])
    ...
    developability = pd.concat([
        ...
        #calc_tap(schro_prot_desc),
        calc_tada(schro_prot_desc)
    ], axis=1)
